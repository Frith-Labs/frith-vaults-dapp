...
function Screen() {
  ...
  // Previews
  const depAmount = useMemo(() => { try { return parseUnits(depInput || '0', aDec) } catch { return 0n } }, [depInput, aDec])
  const redShares = useMemo(() => { try { return parseUnits(redInput || '0', vDec) } catch { return 0n } }, [redInput, vDec])
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <Card title="Deposit (assets → shares)">
          <div className="flex items-end gap-2">
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  // If allowance < amount, do a one-time "infinite" approval and WAIT for it to mine
                  if ((allowance ?? 0n) < depAmount) {
                    ...
                    await waitForTransactionReceipt(config, { hash: approveHash as `0x${string}` })
                  }

                  // Then deposit (will succeed immediately if allowance is now set)
          ...
          {/* Optional UX hint */}
          {(allowance ?? 0n) < depAmount && (
            <p className="text-xs opacity-60 mt-2">
              First-time deposit will request a one-time approval. Future deposits skip approval.
            </p>
          )}
        </Card>
